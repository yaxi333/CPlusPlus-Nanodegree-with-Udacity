...
void Game::Run(Controller const &controller, Renderer &renderer,
  ...
  while (running) {
    ...
    Update();
    renderer.Render(snake, ai_snake, food, obstacle);
...
void Game::PlaceFood() {
  ...
  while (true) {
    ...
    // food.
    if (!snake.SnakeCell(x, y) && !ai_snake.SnakeCell(x, y)) {
      food.x = x;
      food.y = y;
      return;
    }
...
void Game::PlaceObstacle() {
  ...
  while (true) {
    ...
    // Check that the location is not occupied by a snake item before placing
    // obstacle.
    if (!snake.SnakeCell(x, y)  && !ai_snake.SnakeCell(x, y) && !(x == food.x && y == food.y)) {
      obstacle.x = x;
      obstacle.y = y;
      return;
    }
...
void Game::Update() {
  ...
  if (ai_snake.alive) {
    ai_snake.UpdateAI(food, snake, obstacle);
    ai_snake.Update();
  }
